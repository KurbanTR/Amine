...
const Details = () => {
  ...
  const hasGenres = anime?.genres?.length > 0;
  const hasAuthors = anime?.authors?.length > 0;
  const hasSynopsis = anime?.synopsis?.length > 0;
        ...
        <div className={s.info}>
          ...
          {hasGenres && (
            <div className='flex pt-7 w-[340px]'>
              ...
              <p className='text-base'>{anime.genres.map((genre) => genre.name).join(', ')}</p>
            </div>
          )}
        ...
        <div>
            ...
            <div className='pt-10'>            
                ...
                <p className='text-[#7c7c7c] text-lg font-medium leading-9'>
                    {limit ? `${anime.synopsis.slice(0, 394)}...` : anime.synopsis}
                </p>
                </div>
            {anime.synopsis.length > 394 && (
                <span
